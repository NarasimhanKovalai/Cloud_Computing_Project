...
static msg_peer_t *listOfUsers[MAX_USERS] = {
	0};  // global array of connected users to the system.
static int port_cnt = 0;
/*Define by me a function that peeks into packet and by her type provide the
 * service(excute the right code)*/
...
int main(int argc, char *argv[])  //***check all if for exit the program or not.
{
	...
	socket_fd = socket(AF_INET, SOCK_STREAM,
			   0);	// SOCK stream used for TCP Connections
	if (socket_fd == -1) {
		printf("Could not create socket, there is something wrong with "
		       "your system");
	}
	puts("Socket created");
	...
	// Listen (opens the socket for income connection up to NUM_OF_USERS
	// connection)
	...
	while (1) {
		...
		// bzero((char *) &client_sock, sizeof(client_sock)); 	//clear!
		// and Prepare the sockaddr_in structure
		...
		if (pthread_create(&t, NULL, handlePacket,
				   (void *)&client_fd) != 0) {
...
void user_delete(msg_down_t *msg) {
	...
	for (i = 0; i < MAX_USERS; i++) {
		if (listOfUsers[i]) {
			if (listOfUsers[i]->m_addr == msg->m_addr &&
			    listOfUsers[i]->m_port == msg->m_port) {
...
void *handlePacket(void *tArgs) {
	...
	if (recv(fd_request, (void *)&msg_type, sizeof(msg_type_t), MSG_PEEK) ==
	    -1) {
	...
	switch (msg_type) {
		case MSG_UP:
			{  // check the client has send MSG_UP and return to the
			   // client MSG_ACK message
				printf("Got MSG_UP message\n");

				msg_up_t msg;
				if (recv(fd_request, (void *)&msg,
					 sizeof(msg_up_t), 0) == -1) {
				...
				// after we fetched the data we need to generate
				// MSG_ACK and send it to the client get client
				// information
				in_addr_t localClientIP =
					msg.m_addr;  // client IP

				/*Get Local Port*/
				struct sockaddr_in local_address;
				socklen_t laddrlen = sizeof(
					local_address);	 // getsockname function
							 // will put the address
							 // and its length in
							 // this var
				getsockname(fd_request,
					    (struct sockaddr *)&local_address,
					    &laddrlen);
				in_port_t localClientPort =
					local_address.sin_port;
				// printf("The ip:port of inComing connection is
				// : %s:%d\n",inet_ntoa(*(struct in_addr
				// *)&localClientIP),localClientPort);
				printf("The username of incoming connection is "
				       ": %s\n",
				       msg.m_name);

				// since we are using a local
				// system(localHost)I've added a increasing
				// number to client port
				localClientPort +=
					++port_cnt;  // add +1 to client port to
				...
				msgToSend.starter_time = rand() % 5;

				/*register the client in our system!*/
				/*produce MSG_PEER messege*/

				/*Send MSG_ACK message*/
				if (send(fd_request, (void *)&msgToSend,
					 sizeof(msgToSend), 0) < 0) {
					perror("Failed to send "
					       "MSG_ACK..exiting..");
				}

				sleep(1);
				msg_peer_t *msgToSaveNServer =
					(msg_peer_t *)malloc(
						sizeof(msg_peer_t));
				msgToSaveNServer->m_addr = localClientIP;
				strcpy(msgToSaveNServer->m_name,
				       msg.m_name);  // copy from msg to Server
				...
				// printf("The ip of first user in array is:
				// %s:\n",inet_ntoa(*(struct in_addr
				// *)&actualArgs->programlist[0]->m_addr));
				// printf("The name of first user in array is:
		...
		case MSG_DOWN:
			{
				printf("Got MSG_DOWN message Removing User "
				       "from SYSTEM\n");
				msg_down_t *msg = (msg_down_t *)malloc(
					sizeof(msg_down_t));
				if (recv(fd_request, msg, sizeof(msg_down_t),
					 0) == -1) {
					perror("read Messege \"DOWN\" fail");
					free(msg);
					exit(1);
				}
				/*This messege will make the server remove the
		...
		case MSG_WHO:
			{
				...
				/*meaning we are send MSG_HDR and than sleep(1)
				 * and than send MSG_PEERS*/
				sleep(1);
				printf("Sending [%d] Users Data please Hold "
				       "On...",
				       users_count);
				...
				for (int i = 0; i < MAX_USERS; i++) {
					...
					if (listOfUsers[i]) {
						msg_peer_t sendMe;
						sendMe.m_addr =
							listOfUsers[i]->m_addr;
						sendMe.m_port =
							listOfUsers[i]->m_port;
						sendMe.m_type =
							listOfUsers[i]->m_type;
						strcpy(sendMe.m_name,
						       listOfUsers[i]->m_name);

						if (send(fd_request, &sendMe,
							 sizeof(msg_peer_t),
							 0) < 0) {
							perror("Failed to send "
							       "MSG_PEER.."
		...
		default:
			{
				printf("\nError! the client has not sent(or "
				       "wrong)messege type at the first field "
				       "of packet!\n");
			}
	}  // end of switchcase
